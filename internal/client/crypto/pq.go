...
// PQKEM holds server enc key; client encapsulates.
type PQKEM struct {
	enc []byte
}

// NewPQKEMFromEnc stores enc key (1184 bytes ML-KEM-768).
func NewPQKEMFromEnc(encKey []byte) (*PQKEM, error) {
	return &PQKEM{enc: encKey}, nil
}

// Encapsulate generates shared secret + ciphertext; caller sends to peer.
...
// GenerateKeyPair ML-KEM-768 key pair (server/agent).
...
// Seal encrypts with key + nonce; appends nonce if not provided.
